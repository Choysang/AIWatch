...
- Production source/post cleanup SQL must never match optional nullable fields with empty-string fallbacks. `coalesce(handle, '') = ''` can match every source without a handle; use exact non-empty `name` / `handle` / `connector_ref` allowlists, inspect the matched names before `DELETE`, and prefer a dry-run SELECT plus a re-runnable strict cleanup transaction.
