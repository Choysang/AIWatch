// Static SKILL.md content: teaches an agent HOW to call the public AI HOT API.
// It must never embed feed data, so fetching the Skill cannot become a data export.

export const SKILL_MD = `---
name: aihot
description: 读取 AI HOT 的全部 AI 动态、精选内容和每日精编日报。匿名免费，无需 API key。适用于 Claude Code、Codex CLI、Cursor、Gemini CLI、GitHub Copilot、OpenCode、Cline、Windsurf 等任意 Agent。
---

# AI HOT Skill

AI HOT 是一个中文 AI 动态精选与日报系统。这个 Skill 只做匿名只读接入：不需要 API Key，不需要 MCP server，不写入任何数据。
...
最新一期 AI HOT 日报。返回 \`{ kind, date, title, summary, reading_path, sections, generated_at }\`。
...
日报索引，返回 \`{ dailies: [{ date, title, summary, generated_at, item_count }] }\`。
